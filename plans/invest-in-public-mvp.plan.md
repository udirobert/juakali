...
- [x] **Decompose monoliths**: cockpit helpers → `cockpit/format.ts`, cockpit styles → `cockpit/investor-cockpit.styles.ts`, ledger styles → `ledger/public-ledger.styles.ts`, help styles → `help/help.styles.ts`, and the app shell's tab chrome → `shell/tabs.tsx` (`NavTabs` + `TabButton`). `investor-cockpit.tsx` ~1731 → 1327, `public-ledger.tsx` 565 → 431, `help.tsx` 647 → 433, `app/index.tsx` 495 → 371.
