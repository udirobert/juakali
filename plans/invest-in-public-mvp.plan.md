...
- **Agentic feel = real initiative, staged theatrically (never simulated)**: the visible motion in the app must start with the agent, not the user. Five primitives:
  - *Arrival moment*: first thing on Home is Jua's latest utterance (live inbound run narration > last digest summary > "I'm watching N deals"), not metrics.
  - *Presence line*: "Jua · last worked 2h ago · 4 runs this week · next digest in 2 days" + open-suggestion count — all derived from real run data; relative timestamps throughout.
  - *Proactive proposals*: hourly cron creates a `proposed` agentRun per commitment when its KPIs go stale (>24h). The card says what Jua *wants* to do with a consequence list; approve → same durable pipeline streams; dismiss → run marked dismissed, no re-proposal inside the staleness window. Nothing runs without approval.
  - *Email as hero*: the inbox address lives inside the Note-to-Jua card as the co-equal path; the collapsed "Email & cadence" panel is gone.
  - *Voice + memory*: first person everywhere ("I logged", "I acted on your approved note"); replies echo the note that started the run ("You said: …"); scorecard carries an opinionated commentary line (vs peer median, % of target, last check-in age) computed from real data.
- Seed data is backdated honestly ("last reported 3 days ago") so Jua's initiative is visible on first visit, and the seeder creates a real open proposal per stale venture.
...
- [x] **Decompose monoliths**: cockpit helpers → `cockpit/format.ts`, cockpit styles → `cockpit/investor-cockpit.styles.ts`, ledger styles → `ledger/public-ledger.styles.ts`, help styles → `help/help.styles.ts`, shell tab chrome → `shell/tabs.tsx`, venture-cockpit styles → `venture-cockpit/venture-cockpit.styles.ts`, investor-onboarding styles → `investor-onboarding/investor-onboarding.styles.ts`. New sizes: `investor-cockpit` ~1731 → 1327, `public-ledger` 565 → 431, `help` 647 → 433, `app/index` 495 → 371, `venture-cockpit` 451 → 340, `investor-onboarding` 789 → 595.
- [ ] **Deferred** `onboarding.tsx` (706) and `admin-dashboard.tsx` (968): their styles share module-level consts (`palette`, `clipBlobStyle`/`glassStyle`/`diagonalPatchStyle`, `WebShapeStyle`) with render code, so a safe hoist needs a full relocate of those consts. Leaving as-is for now — low payoff, elevated risk for internal Lab/funnel dev screens.
